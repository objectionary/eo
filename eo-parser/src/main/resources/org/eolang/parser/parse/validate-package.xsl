...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" id="validate-package" version="2.0">
  ...
  every consumer of the meta takes the first one, so the rest would be
  swallowed and the object would land where the file does not say.
  -->
  <xsl:output encoding="UTF-8" method="xml"/>
  <xsl:template match="/object">
    <xsl:variable name="errors" as="element()*">
      <xsl:for-each select="metas/meta[head='package']">
        <xsl:if test="preceding-sibling::meta[head='package']">
          <error>
            <xsl:attribute name="check" select="'validate-package'"/>
            <xsl:attribute name="line" select="if (@line) then @line else 0"/>
            <xsl:attribute name="severity" select="'critical'"/>
            <xsl:value-of select="'Package is declared more than once'"/>
          </error>
        </xsl:if>
      </xsl:for-each>
    </xsl:variable>
