 ...
 * R-6.5.7 caps the run of consecutive blanks at one; the moment a
 ...
 * two non-blank lines and precedes no object, so R-6.5.7 has nothing to
...
final class LnBlank implements Line {
    ...
    public void into(final Stack stack, final Globals globals, final Emit emit) {
        if (!this.tail && globals.pendingBlanks() >= 1) {
            emit.error(
                this.span.line(), 0,
                "consecutive blank lines forbidden — at most one blank may separate two non-blank lines"
            );
        }
        globals.blank();
    }
