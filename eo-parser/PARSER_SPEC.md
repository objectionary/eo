...
R-3.4.8. **Type annotations on voids.** A **vertical** void may carry **exactly one** type annotation — in an atom, where R-3.4.10 makes every void vertical, and in a plain formation, where the annotation is often the only place a type can be written at all: a formation instantiated solely from Java, its voids filled by a native atom, has no EO call site to infer them from (#6189). Both forms below are optional; an unannotated void is left untyped (its type is inferred), and an annotated void may be followed by an unannotated one. The two forms are mutually exclusive on one void (`a void attribute may carry at most one type annotation`). A bracket void carries no annotation, so a plain formation that wants a typed void ahead of an untyped one must declare both vertically — the head is hoisted first either way (R-3.4.7).
...
- **Callback argument types — `? > name /{type …}`.** The void is itself a formation, filled by whoever fills the enclosing one; the brace list gives the types of *its* void parameters — equivalently, the arguments supplied to that branch. Each member is a `type-var` or a concrete forma, single-space separated, at least one required; the optional marker `?` is **not** allowed inside the list (`? is not allowed inside a /{…} argument list`). Emits an `@args` attribute, members promoted or verbatim per the split. Example: `? > not-found /{string io.file-error}` emits `<o name='not-found' base='∅' args='Φ.string io.file-error'/>`; `? > cant /{Q.string B}` emits `args='Φ.string B'`. (The type-checker treats the callback's result as the atom's own return type; the parser records only `@args`.)

Every `type-var` in one formation — the atom's return signature, a `@type`, or an `@args` member — denotes the same variable when spelled with the same letter, universally quantified over that formation (§3.10.10). So `B` in `cant /{Q.string B}` is the `B` of `? > y /B`.
