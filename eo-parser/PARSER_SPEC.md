...
R-3.4.3. The standalone `^` (`RHO` token) parameter maps to `ρ` and declares the formation's **receiver**; only the *first* parameter may be it (R-3.4.11). The void parameter list therefore accepts `(NAME | PHI | RHO) (SPACE (NAME | PHI))*`. A `^` in any later position is an error (`a ^ void attribute must be the first attribute of its formation`).
...
R-3.4.11. **The receiver is the first void.** A formation may declare the object it is dispatched off as an ordinary void named `^`, written either as the first bracket parameter (`[^ x] > lt`) or as the first body line (`? > ^`); both emit `<o name='ρ' base='∅'/>` (§9.4). It has to be *first*, because every caller fills the receiver first, so a formation that declared one after anything else would take it in the wrong slot. That makes all four of these an error (`a ^ void attribute must be the first attribute of its formation`): a `^` after another bracket parameter (`[x ^]`), a `? > ^` under a head that declares bracket voids (`[x] > foo`), a `? > ^` after another vertical void, and a second `? > ^`. Inside an atom the receiver takes a type annotation like any other void (R-3.4.8, `? > ^ /Q.bytes`), and outside one it takes none.
...
R-3.12.2a. `:^` maps to `ρ`, binding the argument to the receiver the callee declares (R-3.4.11), so `lt x:0 y:^` fills `lt`'s `ρ` with `y`.
...
| A `^` void that is not the formation's first attribute (R-3.4.11) | `a ^ void attribute must be the first attribute of its formation` |
| A `? > ^` line with no formation around it (R-3.4.11) | `a ^ void attribute needs an enclosing formation` |
