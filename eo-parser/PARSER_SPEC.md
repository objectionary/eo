...
R-3.14.2. **Predecessor requirement.** The stack top at the pipe's indent must be a **formation** (`bare-formation` or `inline-phi-formation`) or another **pipe-application**. A formation predecessor may be **nameless**: the pipe supplies the name (R-3.14.7), so `[x]` on its own line followed by `| 5 > y` is legal even though a bare `[x]` is otherwise an illegal (unnamed) formation attribute — the waiver applies only because a pipe consumes it. A **pipe** predecessor must be **named** (it is referred to by name). A pipe with no predecessor (top-level / empty stack), a deeper-indent ("descending") pipe, or a pipe whose predecessor is a plain value, an application, an unnamed pipe, or any `.method` dispatch is an error.
...
R-3.14.7. **Emission / XMIR.** The parser emits the pipe line as a base-less `<o pipe=''>` with the args as children; the `wrap-applications` reshape (§9) then resolves it against the predecessor in one of two ways:
  - **Named predecessor** — the pipe becomes an ordinary application whose `@base` is the predecessor's name, and the predecessor object stays in place. So `| a > r` after a named formation `F` is identical in XMIR to `F a > r`; `| a` then `| b` after `F >>` (auto-name `A`) is `A a` (auto-named) followed by `A′ b`.
  - **Nameless-formation predecessor** — there is no name to refer to, so the pipe **fuses** into the formation: its `@name` (and `@const`) and its argument children move into the formation node, and the pipe node is dropped. The result is the phi-calculus fusion `⟦x↦5⟧` — a single formation node carrying its voids and body attributes plus the applied arguments as trailing (nameless / `@as`) children. This is byte-for-byte the XMIR a `(body > [x]) 5` paren group already produces, so every downstream pass treats it identically.
...
R-5.2.4a. **PipeApplication same-indent line.** A `PipeApplication` line (§3.14) is a Non-MethodDispatch line and so replaces the top per R-5.2.4, but first the top must satisfy R-3.14.2: its kind must be `bare-formation`, `inline-phi-formation`, or `pipe-application`, and — unless it is a formation — it must be named. Otherwise error `a pipe must follow a formation or another named pipe` (§9.9). In particular a top whose kind is `vmethod` / `vmethod-with-hargs` (a `.method` was taken off the predecessor) fails this check — R-3.14.4. When the top is a nameless formation, the transition additionally marks it so its close-time naming check (R-5.3.1) is waived — the pipe supplies its name (R-3.14.7).
...
R-5.2.5a. If the line's kind is `PipeApplication`: error `a pipe must follow a formation or another named pipe` — a deeper-indent ("descending") pipe has no same-indent predecessor to apply to. A `.method` line at indent `N` requires a previous sibling expression at the same indent; a deeper-than-parent position has no such sibling. **This rule is the authoritative owner of the `.method`-as-deeper-line rejection**, including the bare-reversed-receiver edge case (a `.method` line as the first deeper child of a bare-reversed parent). R-5.2.9's "must not start with `.`" condition is enforced *via this rule*; R-5.2.9 itself only manages the `receiver_consumed?` flag.
...
R-5.2.11a. If the line's kind is `PipeApplication`: error `a pipe must follow a formation or another named pipe` — an empty stack has no object above to apply to.
