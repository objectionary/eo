...
R-3.4.7. A void attribute may also be declared **vertically** as a `? > name` body line of the formation (the `?` is the `VOID` token of §2.3). It is equivalent to listing `name` among the bracket parameters: it emits the same void child (§9.4), and `move-voids-up` hoists it among the head voids in source order, so `[name] > foo` with body lines `? > bar` and `? > test` is identical in XMIR to `[name bar test] > foo`. The `>>` auto-name suffix (§3.10) is also accepted: `? >> name` declares a void whose external `@name` is an auto-generated cactus name (§9.2, unreachable from outside), while `name` is a *file-local handle* (R-3.10.12) usable within the same `.eo` file; a bare `? >>` is a void with an auto-generated name and no handle. Filling stays positional, so the auto-generated external name does not affect how callers bind the void. `? > name` and `? >> name`, each optionally followed by one type annotation (§3.4.8), are the **only** shapes the `?` marker may take: the marker is not a value, so it may not appear as an argument (`foo ? bar`), a method receiver (`?.read`), or a reversed-dispatch argument (`foo. ? q`), and a bare `?` or any other trailing tokens are an error. The form requires a name (or auto-name) suffix and is legal only as a direct child of a formation, which has no children of its own (a deeper-indent line under a void is rejected). A `!` const marker on a void is rejected (`a void attribute must be written as ? > name or ? >> name`); a type annotation, however, is permitted (§3.4.8). Reverse printing canonicalises a void to the bracket form, since the two are indistinguishable in XMIR — except where the bracket form cannot carry what the void holds: an auto-name handle (R-3.10.12), a type annotation (R-3.4.8), or membership in an atom (R-3.4.10). Each of those prints vertically and takes every sibling void with it, because the head is hoisted ahead of the body and a split would reorder the two groups.
R-3.4.8. **Type annotations on voids.** A **vertical** void may carry **exactly one** type annotation — in an atom, where R-3.4.10 makes every void vertical, and in a plain formation, where the annotation is often the only place a type can be written at all: a formation instantiated solely from Java, its voids filled by a native atom, has no EO call site to infer them from (#6189). The annotation is a **union**: one or more members naming the types the void may hold, in source order. It is optional; an unannotated void is left untyped (its type is inferred), and an annotated void may be followed by an unannotated one. A second `/` on one void is an error (`a void attribute may carry at most one type annotation`) — every type belongs in the one union. A bracket void carries no annotation, so a plain formation that wants a typed void ahead of an untyped one must declare both vertically — the head is hoisted first either way (R-3.4.7).

```
void-anno ::=  '/' union '?'?                   (trailing ? = maybe-⊥, closes the annotation)
union     ::=  member ('|' member)*             (no spaces around the pipes)
member    ::=  type | '{' (type (SPACE type)*)? '}'
...
- **Formation type — `? > name /{type …}`.** The void is itself a formation, filled by whoever fills the enclosing one; the brace list gives the types of *its* voids — equivalently, the arguments supplied to that branch when the void is a callback. Each entry is a `type-var` or a concrete forma, single-space separated; `{}` names a formation with **no** voids, the shape an anonymous empty object takes. A formation type does **not** nest — its entries are plain types, so a second `{` before the closing brace is an error (`a /{…} formation type cannot nest another one`). The optional marker `?` is **not** allowed inside the braces (`? is not allowed inside a /{…} formation type`), and an unclosed brace list is an error (`a /{…} formation type must end with }`). The braces survive into the emitted `@type`, their entries promoted or verbatim per the split. Example: `? > not-found /{string io.file-error}` emits `<o name='not-found' base='∅' type='{Φ.string io.file-error}'/>`; `? > cant /{Q.string B}` emits `type='{Φ.string B}'`. (The type-checker treats a callback's result as the atom's own return type; the parser records only the shape.)
- **Union — `? > name /one|two|three`.** Every annotation is a union; the two bullets above describe its members, and a `|` adds another one. A union of one is the plain form (`? > position /Q.number`), a union of several says the void may hold **any** of them — the only way to describe a void a native atom fills differently per call, like the `output` of a syscall's `return`. Members are emitted in source order, separated by a single space, so a reader splits on spaces *outside* braces. Example: `? > output /{}|Q.bytes|Q.posix.stat` emits `type='{} Φ.bytes Φ.posix.stat'`. A `|` with no type on either side is an error (`a void type annotation requires a type`), spaces around a `|` are one too (`a union must have no spaces around its pipes`), and the trailing `?` closes the whole annotation rather than one member (`the optional marker ? must close a void type annotation`).

Every `type-var` in one formation — the atom's return signature, or any member of a void's union — denotes the same variable when spelled with the same letter, universally quantified over that formation (§3.10.10). So `B` in `cant /{Q.string B}` is the `B` of `? > y /B`.
...
| generic type variable `A`–`F` | (verbatim) | `@atom`, any `@type` member — never `Φ`-promoted or alias-expanded (§3.10.11) |
...
| Void type `? > name /one\|two` | `@type='<member> …'` on the void's `<o>`: the union's members in source order, separated by a single space, each a `Φ`-promoted concrete forma, a verbatim variable A–F, or a braced formation type whose own entries are space-separated; any trailing `?` preserved, applying to the whole union (R-3.4.8) |
...
| `?` inside a `/{…}` formation type | `? is not allowed inside a /{…} formation type` |
| Unclosed `/{…}` formation type | `a /{…} formation type must end with }` |
| Nested `/{…}` formation type | `a /{…} formation type cannot nest another one` |
| Spaces around a union's `\|` | `a union must have no spaces around its pipes` |
| `?` before the end of a union | `the optional marker ? must close a void type annotation` |
