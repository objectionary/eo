...
final class FoldedTest {
    ...
    void foldsBinaryOperationOverLiterals(final String lambda, final String operands,
        final String phi) {
        MatcherAssert.assertThat(
            String.format("the %s of two literals must fold to data, but it didnt", lambda),
            new Folded(new Op(lambda), Arrays.asList(operands.split(" ", -1))).phi(),
    ...
    void foldsBytesNot() {
        MatcherAssert.assertThat(
            ...
            new Folded(new Op("L_bytes_not"), Collections.singletonList("bytes:F0-0F")).phi(),
    ...
    void foldsBytesSizeToNumber() {
        MatcherAssert.assertThat(
            ...
            new Folded(new Op("L_bytes_size"), Collections.singletonList("bytes:01-02-03")).phi(),
    ...
    void leavesUnknownOperationUnfolded() {
        MatcherAssert.assertThat(
            ...
            new Folded(
                new Op("L_bytes_right"),
                Arrays.asList("bytes:01-02", "number:40-00-00-00-00-00-00-00")
            ).phi(),
