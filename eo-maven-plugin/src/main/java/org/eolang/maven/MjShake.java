...
public final class MjShake extends MjSafe {
     ...
     * If {@link MjSafe#trackTransformationSteps} is {@code true}, we create a new {@link Xsline}
        ...
        if (this.trackTransformationSteps) {
            func = xml -> new Xsline(
                new TrSpy(
                    measured,
                    new StickyFunc<>(
                        new ProgramPlace(this.targetDir.toPath().resolve(MjShake.STEPS))
                    )
                )
            ).pass(xml);
        } else {
