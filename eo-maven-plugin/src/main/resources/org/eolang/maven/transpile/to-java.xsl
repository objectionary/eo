...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eo="https://www.eolang.org"
  ...
  <xsl:param name="disclaimer"
             select="'This file was auto-generated by eo-maven-plugin, your changes will be discarded on the next build'"/>
  ...
  <xsl:function name="eo:clean" as="xs:string">
    ...
    <xsl:value-of
      select="concat('EO', replace(replace(translate(translate(replace($n, '_', '__'), '-', '_'), '@', $eo:phi), $eo:alpha, '_'), '\$', '\$EO'))"/>
  </xsl:function>
  ...
  <xsl:function name="eo:loc-to-class">
    <xsl:param name="loc"/>
    <xsl:value-of
      select="concat('EO', replace(translate(string-join(tokenize($loc, '\.'), ''), '-', '_'), $eo:cactoos, $eo:alpha))"/>
  </xsl:function>
  ...
  <xsl:template match="class" mode="testing-ctors">
    <xsl:variable name="class"
                  select="concat(eo:class-name(@name, eo:suffix(@line, @pos)), 'Test')"/>
    <xsl:text>/**</xsl:text>
    <xsl:value-of select="eo:eol(1)"/>
    <xsl:text> * Ctor.</xsl:text>
    <xsl:value-of select="eo:eol(1)"/>
