...
/**
 * Test case for {@link MjLatex}.
 *
 * @since 0.29
 */
@ExtendWith(MktmpResolver.class)
final class MjLatexTest {
    ...
    void generatesTexFile(@Mktmp final Path temp) throws Exception {
        MatcherAssert.assertThat(
            ...
            new FakeMaven(temp)
                ...
                .result(),
            Matchers.hasKey(
                String.format("target/%s/main.%s", MjLatex.DIR, MjLatex.EXT)
            )
        );
    }
        ...
        final String output = String.join(
            ...
            Files.readAllLines(
                new FakeMaven(temp)
                    ...
                    .execute(new FakeMaven.Latex())
                    .result()
                    .get(String.format("target/%s/main.%s", MjLatex.DIR, MjLatex.EXT))
            )
        );
        MatcherAssert.assertThat(
