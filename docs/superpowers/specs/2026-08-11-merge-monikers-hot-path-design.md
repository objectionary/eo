...
The stylesheet will declare XSLT 3.0 and enable `cache="yes"` on
`eo:moniker-refs` and `eo:hosted-binding`. Both functions are pure within one
transformation and are repeatedly called with the same source nodes. Caching
their results removes the repeated key lookups, filtering, and dispatch
ordering while retaining node identity and document order.
...
Saxon-HE 13.0 supports XSLT 3.0 stylesheet functions with `cache="yes"`. The
stylesheet has no side effects, and the cached functions neither depend on an
implicit context item nor construct new trees. Their results therefore remain
safe to reuse during one transformation.
...
optimization contract: XSLT 3.0, caching on the two hot functions, cheap guards
before the two expensive template predicates, and a conditional sort. The test
will be run before implementation to confirm it fails for the missing
optimizations.
...
1. The focused regression test fails before the stylesheet change and passes
   afterward.
2. `eo:moniker-refs` and `eo:hosted-binding` reuse cached results during a
   transformation.
3. Irrelevant nodes are rejected by cheap template-pattern predicates before
   the hosted/applied lookup functions run.
