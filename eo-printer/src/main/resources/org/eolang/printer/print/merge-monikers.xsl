...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eo="https://www.eolang.org" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" id="merge-monikers" version="2.0">
  ...
  For each formation, every named bound attribute that is neither void,
  `φ`, a test, nor pipe-floated is merged onto the first bare `ξ.<name>`
  reference reachable through no intervening formation (so the name still
  binds to the same formation). A method-dispatch use such as `value.gte`,
  a reference carrying arguments, or a reference that is itself a named
