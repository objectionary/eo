...
\T{head} \df \T{name} \alt \T{data} \alt \V{@} \alt \V{\$} \alt \V{\&} \\
...
\ff{int} & \ff{-42} & 4 \\
\ff{float} & \ff{3.1415926} or \ff{2.4e-34} & 4 \\
...
the object \ff{walk}. Most probably it will point to the \ff{walk} object itself,
but this solely depends on the implementation of \ff{walk}.
...
However, using \ff{\^{}.f} won't work, because the parent of it is a copy of \ff{mapi}, and the
parent of \ff{mapi} is the object \ff{array}. There is no way to get access
to \ff{map.f} using parent attributes.

The \emph{home} attribute \ff{\&} helps here. In each object it is bound to the parent
object the object got first time. The parent of the inner object at \lref{map-inner}
was set to \ff{map} when the object was defined. Then, \ff{mapi} changes it on each
iteration, setting to itself, but home forever remains the same.
