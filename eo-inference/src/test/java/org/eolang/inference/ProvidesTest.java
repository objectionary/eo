...
final class ProvidesTest {
    ...
    void marksFormationComplete() {
        MatcherAssert.assertThat(
            ...
            new TjSmart(
                new Provides(
                    List.of(new XMLDocument("<object><o loc='Φ.lid' name='lid'/></object>"))
                ).rows()
    ...
    @Test
    void putsRowsOfManyFilesInOneTable() {
        MatcherAssert.assertThat(
            "the formations of every file must land in the same table, but they didnt",
            new Provides(
                List.of(
                    new XMLDocument("<object><o loc='Φ.jar' name='jar'/></object>"),
                    new XMLDocument("<object><o loc='Φ.pot' name='pot'/></object>")
                )
            ).rows().select(row -> row.exists("complete")),
            Matchers.hasSize(2)
        );
    }
}
